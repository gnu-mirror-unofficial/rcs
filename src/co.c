...
#		endif
        ...
        if (!Head)
        ...
        else
          {
            ...
            if (!
                (targetdelta =
                 genrevs (numericrev.string, date, author, state,
                          &gendeltas)))
              continue;
            /* check reservations */
            changelock =
  ...
  if (workstat.st_mode & (S_IWUSR | S_IWGRP | S_IWOTH) && !forceflag)
    {
      ...
      if (!yesorno (false, "writable %s exists%s; remove it? [ny](n): ",
                    workname,
                    myself (workstat.
                            st_uid) ? "" : ", and you do not own it"))
        {
          error (!quietflag && ttystdin ()? "checkout aborted"
      ...
      else
        {
          if (lastjoin == 0)
            {                   /* first pair */
              ...
              if (!
                  (joinlist[0] = getancestor (targetdelta->num, joinlist[1])))
                return false;
            }
          else
            {
